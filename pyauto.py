...
init(autoreset = True)
version = "1.3" # neu co chinh sua noi dung vui long nhap version
...
def chuc_nang_2():
    print(Fore.RED + "Cảnh báo: Đảm bảo rằng máy tính của bạn có ứng dụng Notepad để chương trình hoạt động, nếu không, chương trình có thể gây ra bất kì hanh vi nào không mong muốn" + Fore.RESET)
    while True:
        sleep(2)
        content = input("Nhập nội dung cần ghi vào (nhập '0' để thoát chương trình): ").strip()
        if content == "'0'":
            print(Fore.RED + "Đã dừng chương trình do giá trị nhập là '0'" + Fore.RESET)
            sleep(3)
            return
        countdown(3)
        print("\n")
        try:
            ...
            print("Đã ghi xong nội dung vào Notepad.")
            sleep(1)
            close = input("Bạn có muốn đóng ứng dụng Notepad không? (y / n) : ").strip().lower()
            if close == "y":
                open_app("Notepad")
                print("Đã mở Notepad;")
                sleep(0.7)
                ...
                sleep(0.3)
                pyautogui.press("n")
                print("Đã đóng Notepad")
            elif close == "n":
                pass
            else:
                print("Hãy nhập đúng giá trị chương trình yêu cầu")
        except pyautogui.FailSafeException:
            print(Fore.RED + "Đã dừng chương trình gấp" + Fore.RESET)
            sleep(2)
            return
        ...
        while True:
            ...
            if su == "y":
                ...
                try:
                    ...
                    sleep(8)
                    clos = input("Bạn có muốn đóng nó không? (y / n) : ").strip().lower()
                    if clos == "y":
                        print("Bây giờ mới cảnh báo nè : ", end = "")
                        print(Fore.RED + "Nếu bạn đồng ý với thao tác trên, nó sẽ xóa hết tab trong trình duyệt của bạn" + Fore.RESET)
                        cl = input("Bạn có chắc chắn đóng ứng dụng không? (y / n): ").strip().lower()
                        if cl == "y":
                            sleep(0.5)
                            open_app("Edge")
                            print("Đã mở Edge")
...
def chuc_nang_4():
    try:
        print(Fore.RED + "Cảnh Báo: Ở chương trình này, ta không thể dừng gấp bằng cách đưa con trỏ chuột về góc trái phía trên màn hình.\nĐể dừng chương trình (auto), ta có thể nhấn Ctrl + C để thoát chương trình, hoặc bấm lại tổ hợp đó thêm 1 lần nữa để dừng hẳn chương trình.\nNhập 0 vào các input để thoát chương trình."+ Fore.RESET)
        sleep(5)
                ...
                if tg == "+":
                    while True:
                        ...
                        if t < 0:
                            ...
                            print(f"Volume hiện tại: {round(current*100)}")
                            countdown(5)
                            for i in range(t):
                                ...
                                sleep(0.1)
                            print(f"Đã tăng xong {t} âm lượng\nÂm lượng hiện tại của bạn : {round(current*100)}")
                            sleep(5)
                            break
                elif tg == "-":
                    while True:
        ...
        if log == "y":
            ...
            try:
                with open("NetCheck.txt", "a", encoding="utf-8") as f:
                    ...
                    f.write("="*40 + "\n\n")
                print(Fore.GREEN + "Đã lưu kết quả vào NetCheck.txt" + Fore.RESET)
                sleep(3)
            except Exception:
